...
MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::navierstokes_2d_incompressible_homogeneous()
{
  this->mmsname = "navierstokes_2d_incompressible_homogeneous";
  this->dimension = 2;
...
template <typename Scalar>
int MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::init_var()
{
  int err = 0;

  err += this->set_var("a",1.05);
  err += this->set_var("b",-1.05);
  err += this->set_var("c",10.1);
...
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_q_u(Scalar x1, Scalar y1, Scalar z1)
{
  ...
  U[0]       = a * helper_f(beta,kx,x)                  * helper_g(delta,ky,y).derivatives()[1];
  U[1]       = b * helper_f(beta,kx,x).derivatives()[0] * helper_g(delta,ky,y);
  U[2]       = 0.0;
  ADScalar P = c * helper_f(beta,kx,x)                  * helper_g(delta,ky,y);
...
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_q_v(Scalar x1, Scalar y1, Scalar z1)
{
  typedef DualNumber<Scalar, NumberVector<NDIM, Scalar> > FirstDerivType;
  typedef DualNumber<FirstDerivType, NumberVector<NDIM, FirstDerivType> > SecondDerivType;
  typedef DualNumber<SecondDerivType, NumberVector<NDIM, SecondDerivType> > ThirdDerivType;
...
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_q_w(Scalar x1, Scalar y1, Scalar z1)
{
  typedef DualNumber<Scalar, NumberVector<NDIM, Scalar> > FirstDerivType;
  typedef DualNumber<FirstDerivType, NumberVector<NDIM, FirstDerivType> > SecondDerivType;
  typedef DualNumber<SecondDerivType, NumberVector<NDIM, SecondDerivType> > ThirdDerivType;
...
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_exact_u(Scalar x, Scalar y1, Scalar z1)
{
  ...
  Scalar exact_u;
  exact_u =   a *  helper_f(beta,kx,x) * helper_g(delta,ky,y).derivatives();
  return exact_u;
}

// public method
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_exact_v(Scalar x1, Scalar y, Scalar z1)
{
  typedef DualNumber<Scalar, Scalar> OneDDerivType;
  ...
  Scalar exact_v;
  exact_v = b * helper_f(beta,kx,x).derivatives() *  helper_g(delta,ky,y);
  return exact_v;
}

// public method
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_exact_w(Scalar x1, Scalar y1, Scalar z)
{
  typedef DualNumber<Scalar, Scalar> OneDDerivType;
  OneDDerivType x = OneDDerivType(x1,1);
  OneDDerivType y = OneDDerivType(y1,1);
...
// public method
template <typename Scalar>
Scalar MASA::navierstokes_2d_incompressible_homogeneous<Scalar>::eval_exact_p(Scalar x, Scalar y, Scalar z)
{

  Scalar P = c *  helper_f(beta,kx,x) * helper_g(delta,ky,y);
  return P;
}
...
MASA_INSTANTIATE_ALL(MASA::navierstokes_2d_incompressible_homogeneous);
